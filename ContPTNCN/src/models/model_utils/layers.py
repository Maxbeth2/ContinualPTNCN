...
import sys
from models.model_utils.utils import gelu1, softmax, init_weights, gte, ltanh, d_relu6, standardize
import numpy as np
...
class Z:
    ...
    def pred(self, t=None, b=None):
        pass
        # self.f = self.act_fx(self.a) # activate??
    ...
    def guess(self):
        self.p = tf.matmul(self.f, self.W)
...
class ZB(Z):
    def __init__(self, sdim,init_type="normal", out_fx="softmax"):
        super().__init__(sdim=sdim)
        self.out_fx = out_fx
        self.output = None
        # no params
    ...
    def pred(self):
        # self.output = self.out_fx(self.parent.p)
        self.output = softmax(self.parent.p)
        return(self.output)
